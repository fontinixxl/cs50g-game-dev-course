...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        -- last highlighted position in the grid
        self.lastHighlightX = -1
        self.lastHighlightY = -1
        if love.mouse.wasitpressed() then
            ...
            if Board:pointingIn(mouseXBoard, mouseYBoard) then
        ...
        -- if we've pressed enter or the same tile with the mouse, select or deselect a tile...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return')
            or (self:tileIsSelectedTwice()) then
            -- if same tile as self.currently highlighted, deselect
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- if there is a match tween coordinates between the two so they swap
                if self.board:calculateMatches() then
                    ...
                    -- no match: swap back to the original position
                    gSounds['error']:play()
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- add score for each match depending on the variety of every tile in the match.
        for k, match in pairs(matches) do
            for unused, tile in pairs(match) do
                -- Option 1: 50 + 10*variety
            ...
            -- TODO: give more time depending on the tile variety
    ...
    -- after tilesToFall
    Timer.after(0.251, function()
        -- if no matches available to perform, reset the board!
        if not self.board:matchesAvailable() then
            --TODO: use different sound for resetting the board
            gSounds['next-level']:play()
            --TODO: show the user the board has been resetted
...
function PlayState:tileIsSelectedTwice()
    return     self.lastHighlightX == self.boardHighlightX
           and self.lastHighlightY == self.boardHighlightY
end
