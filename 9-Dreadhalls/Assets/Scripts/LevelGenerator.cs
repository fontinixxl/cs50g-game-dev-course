...
public class LevelGenerator : MonoBehaviour {
	...
	// num of holes where the player can fall through
	...
	void Start () {
		...
		// initialize map 2D array of holes
		bool[,] holesData = GenerateHolesData();
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// generate floor if there is not a hole
				if (holesData[z, x] == false) {
	...
	// Generate a 2D boolean array with true values where it has to be a hole.
	bool[,] GenerateHolesData() {

		bool[,] holes = new bool[mazeSize, mazeSize];

		// initialize all holes to false
		for (int y = 0; y < mazeSize; y++) {
			for (int x = 0; x < mazeSize; x++) {
				holes[y, x] = false;
			}
		...
		// counter to represent the holes already marked in the array
		int holesGenerated = 0;

		int randomX = 0, randomY = 0;

		while (holesGenerated < holesToSpawn) {
			randomX = (int)Random.Range(1, mazeSize - 1);
			randomY = (int)Random.Range(1, mazeSize - 1);

			// mark as hole if there is is suposed to be floor
			if (mapData[randomX, randomY] == false) {
				holes[randomX, randomY] = true;
				holesGenerated ++;
			}
		}
