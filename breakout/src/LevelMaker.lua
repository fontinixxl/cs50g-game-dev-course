...
function LevelMaker.createMap(level)
    ...
    -- whether we want to add a Locker Brick in the Level
    local lockedBrickFlag = level % 5 == 0 and true or false
    ...
    -- add a Locked Brick in case we are in a multiple of five level
    if #bricks == 0 then
        return self.createMap(level)
    else
        if level % 5 == 0 then
            lockedBrickPos = math.random(1, table.getn(bricks))
            bricks[lockedBrickPos].color = 6
            bricks[lockedBrickPos].tier = 1
            bricks[lockedBrickPos].breakable = false
        end
